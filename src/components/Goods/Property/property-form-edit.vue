...
<script>
  export default {
    ...
    watch: {
        formData(value) {
            if (this.skuIds.length < this.skuList.length) {
                this.skuIds = this.skuList.map((item) => item.id);
            }
    ...
    methods: {
      ...
      loopProps(data) {
        ...
        propsArray.forEach((item, index) => {
            ...
            this.propsForm.tGoodsSkuList.push({
                title: item,
                // skuCode: `${this.goodsData.goodsCode}-${_index}`,
                skuCode: '系统自动生成',
                barcode: '',
                assitBarcode: '',
                extNo: '',
                ...
                barCodeValidator: this.$Regs.RegMin2MaxAndCheckWithOutCN(3, 50, {
                    key: 'barcode',
                }, oldTGoodsSkuList, this.skuIds.length > index ? this.skuIds[index] : ''),
        ...
        this.propsForm.tGoodsSkuList.forEach((item ,index) => {
            for (const sku of this.skuList) {
                if (item.title.join().indexOf(sku.propertyTplList[0].provalueValues) !== -1) {
                    this.propsForm.tGoodsSkuList[index] = {
                        ...sku,
                        title: item.title,
                        costValidator: item.costValidator,
                        barCodeValidator: item.barCodeValidator,
                        assitBarCodeValidator: item.assitBarCodeValidator,
                        extNoValidator: item.extNoValidator,
                    };
                    break; 
                }
            }
      ...
      setCheckedPropsArray(propsArray) {
          let arr = [];
      ...
      setProps() {
          const format = (name, selectedProps) => {
              this.propsForm.tGoodsSkuList.forEach((item) => {
                ...
                //     //   debugger
                //       if (item.title.join().indexOf(sku.propertyTplList[0].provalueValues) === -1) {
                        if (item.title.findIndex((str) => str === selectedProps) !== -1 && item.properties.indexOf(selectedProps) == -1) {
      ...
      setPropsList() {
          // TODO 编辑时候
            const format = (propertyId, propObj) => {
                // const filterObj = this.skuList.map(item => item.propertyTplList[0].provalueValues);
                this.propsForm.tGoodsSkuList.forEach((item) => {
                    // let flag = true;
                    // for (const i of filterObj) {
                    //     console.log(item);
                    //     if (item.title.join().indexOf(i) !== -1) {
                    //         flag = false;
                    //         break;
                    //     }
                    // }
                    // if (flag) {
                        const filterObj = item.propertyTplList.map(obj => obj.provalueValue)
                        if (item.title.findIndex((str) => str === propObj.provalueValue) !== -1 && filterObj.findIndex((str) => str === propObj.provalueValue) == -1) {
