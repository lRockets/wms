<template>
  <div class="supplier_management">
    ...
    <el-table v-loading="loadingTable.sales" 
      ...
      <el-table-column align="center" prop="status" label="库内状态">
           <template slot-scope="scope">
    ...
    <div class="tabBox">
      <el-tabs v-model="tabActive" 
        ...
        <el-tab-pane label="出库明细" name="ouputDetail">
          <el-table ref="ouputDetailRef" 
                    border
                    :data="outputData">
            <el-table-column type="index" align="center" label="行号" width="60"></el-table-column>
            <el-table-column align="center" prop="skuCode" label="商品SKU"></el-table-column>
            <el-table-column align="center" prop="goodsName" label="商品名称"></el-table-column>
            <el-table-column align="center" prop="locationCode" label="出库库位"></el-table-column>
            <el-table-column align="center" prop="batchNo" label="入库批次"></el-table-column>
            <el-table-column align="center" prop="goodsSerialNumber" label="序列号"></el-table-column>
          </el-table>
        </el-tab-pane>
        <el-tab-pane label="操作记录" name="operate">
          <el-table ref="operateRef" 
                    border
                    :data="operateData">
            <el-table-column align="center" prop="status" label="订单状态">
             ...
             <el-table-column align="center" prop="optTime" label="操作时间"></el-table-column>
            <el-table-column align="center" prop="optUsername" label="操作人"></el-table-column>
            <el-table-column align="center" prop="logContent" label="操作日志"></el-table-column>
          </el-table>
        </el-tab-pane>
        <el-tab-pane name="total" disabled>
          <p slot="label">总数量为：
            <span>{{orderDetail.saleoutQty}}</span>
            总金额为：
            <span>{{orderDetail.saleoutAmount}}</span>
          </p>
        </el-tab-pane>
      </el-tabs>
  ...
  export default {
    ...
    methods: {
      ...
      async getMainList(params) {
        ...
        const { data } = await this.$ajaxPost('wms/saleout/selectPage.do', {
        ...
        if (data.code === 200) {
          ...
          if (this.tableData.length !== 0) {
            ...
            this.prevSaleoutNo = this.tableData[0].saleoutNo;
            //默认选择第一行
            this.$refs.salesList.setCurrentRow(this.tableData[0]);
            // 判断是否有编辑权限
            this.canEdit = data.data.userRole;
            ...
            this.getOperateDetail(this.prevSaleoutNo);
            this.getOutputDetail(this.prevInventoryId);
          } else {
            this.loadingTable.inventory = false;
            //搜索为空时  清空商品明细，订单明细，备注/留言，发票信息，出库明细，操作记录
       ...
       * @param { int } id: 当前销售单的id值
       */
      async getOperateDetail(saleoutNo) {
        const { data } = await this.$ajaxPost('wms/saleoutLog/saleoutNoLog.do', {
          reqParam: JSON.stringify({
            saleoutNo,
          }),
        });
        if (data.code === 200) {
          this.operateData = data.data;
        } else {
          this.$message.error(data.msg);
        }
      },
       ...
       * @param { int } id: 当前销售单的id值
       */
      async getOutputDetail(id) {
        const { data } = await this.$ajaxPost('wms/tOutserialNumber/selectBySaleoutId.do', {
          reqParam: JSON.stringify({
            id,
          }),
        });
      ...
      async cancelInterCeptOrder(ids) {
        const { data } = await this.$ajaxPost('wms/saleout/updateCancelIntercept.do', {
          reqParam: JSON.stringify({
            ids,
          }),
        });
      ...
      async deleteOrder(ids) {
        const { data } = await this.$ajaxPost('wms/wavepickDetail/allcloseOrder.do', {
          reqParam: JSON.stringify({
            ids,
          }),
        });
      ...
      // 取消拦截订单
      cancelInterceptOrderBtn(){
        if (this.ids.length > 0) {
          this.cancelInterCeptOrder(this.ids.join());
        } else {
          this.$message.warning('请至少选择一条数据');
        }
      },
      ...
      rowClick(row) {
        if (this.prevInventoryId !== row.id) {
          ...
          this.getOperateDetail(row.saleoutNo);
          this.getOutputDetail(row.id);
