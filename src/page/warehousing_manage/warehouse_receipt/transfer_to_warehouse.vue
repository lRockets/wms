<template>
  <div class="transferWarehouse">
    <div class="transfer" v-if="isDetail" key="transfer">
      ...
      <el-table ref="transferToList" border :data="transferToList" height="500px" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" prop="name" label="操作" width="110"> 
          <template slot-scope="scope">
            <el-button type="text" @click="checkTranDetail(scope.row)">查看详情</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column :show-overflow-tooltip="true" align="center" prop="alloinNo" label="预约单号"></el-table-column>
    ...
    <div class="transferDetail" v-else key="transferDetail">
      <header class="top-head">
     ...
     <el-dialog :title="storehouseTitle" :visible.sync="storehouseDia" height='400px'  :modal-append-to-body="false">
      <el-form :model="storehouseData" label-width="110px" class="detailInfor storehouse">
       ...
       <el-table ref="storehouseList2" border :data="storehouseList2" height="500px" >
        ...
        <el-table-column :show-overflow-tooltip="true" align="center" prop='inboundQty' label="实际入库数量">
          <template slot-scope="scope">
  ...
  export default {
    ...
    methods:{
      ...
      getTransferList(param){
        ...
        this.$ajaxPost('wms/inbound/alloinbound/selectSearchPage.do',{reqParam:json_param}).then(({data})=>{
          if(data.code==200){
            this.transferToList=data.data.dataList
      ...
      //批量导出
      batchExport(){
        this.$refs.exportFile.export('调拨入库单','/inbound/alloinbound/selectForExport.do',this.param,this.filterParam)
      },
      //快速搜索
      qSearch(){
      ...
      checkTranDetail(row){
        ...
        this.detailInfor=this.transferRowData;
        this.$nextTick(()=>{
          this.getDetailInfor();
        })
      },
      ...
      getDetailInfor(){
        let json_param = JSON.stringify({
          ...
          inboundId:this.transferRowData.id,
          keyword:this.$refs.detail_qSearch.inputValue})
        this.$ajaxPost('wms/inbound/alloinboundDetail/selectPageByInboundIdForPC.do', {reqParam: json_param}).then(({data}) => {
          if (data.code == 200) {
      ...
      editDetail(scope){
        ...
        this.$ajaxPost('wms/inbound/alloinboundDetail/selectNotPageByInboundIdAndSkuId.do',{reqParam:json_param2}).then(({data})=>{
          if(data.code==200){
            this.getTransferDetail(scope.row)
          }else{
            this.$message.error(data.msg)
            return false;
          }
      ...
      getTransferDetail(GoodsRow){
        ...
        this.$ajaxPost('wms/inbound/alloinboundDetail/selectPageByInboundIdAndSkuId.do',{reqParam:json_param}).then(({data})=>{
          if(data.code==200){
      ...
      submitEdit(){
        ...
        if(flag){
          let json_param=JSON.stringify({alloinboundDetailList:detailList})
          this.$ajaxPost('wms/inbound/alloinboundDetail/updateDetailById.do',{reqParam:json_param}).then(({data})=>{
            if(data.code==200){
